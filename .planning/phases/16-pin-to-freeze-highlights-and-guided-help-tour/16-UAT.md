...
number: 2
name: Pinned highlight doesn't change when hovering other chunks
expected: |
  While a chunk is pinned (ring + hint visible), hover other layer chunks. The canvas
  highlight should NOT change — it stays frozen on the originally-pinned layer.
